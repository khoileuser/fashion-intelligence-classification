...
function SimilarCard({ item, rank }: { item: SimilarItem; rank: number }) {
    ...
    const title =
        item.productDisplayName || item.articleType || `Catalogue item ${id}`
    ...
    return (
        <Card className="gap-0 overflow-hidden py-0">
            <div className="relative aspect-3/4 overflow-hidden bg-muted">
                ...
                <img
                    src={`/api/gallery/${encodeURIComponent(id)}/image`}
                    alt={String(title)}
                    className="h-full w-full object-contain transition-transform duration-300 hover:scale-[1.02]"
                />
                <Badge className="absolute left-3 top-3 bg-background text-foreground shadow-sm">
                    #{rank}
                </Badge>
            </div>
            <CardContent className="space-y-3 p-4">
                ...
                <div className="flex min-h-6 flex-wrap gap-1.5">
                    {[item.gender, item.season, item.usage]
                        .filter(Boolean)
                        .map((tag) => (
                            <Badge variant="secondary" key={String(tag)}>
                                {String(tag)}
                            </Badge>
                        ))}
                </div>
                <div className="flex items-center justify-between border-t pt-3 text-xs">
                    <span
                        ...
                        title="Cosine similarity ranks visual features; it is not a probability of a correct match."
                    >
                        Similarity
                    </span>
...
export function FashionAnalyser({ initialSample }: { initialSample?: string }) {
    ...
    const [busy, setBusy] = useState(Boolean(initialSample))
    ...
    const runAnalysis = useCallback(async (file: File, signal: AbortSignal) => {
        setBusy(true)
        setError(null)
        setResult(null)
        ...
        try {
            const response = await fetch(
                "/api/analyse?prediction_top_k=3&search_top_k=5",
                {
                    method: "POST",
                    body: formData,
                    signal,
    ...
    useEffect(() => {
        ...
        fetch(`/api/gallery/${initialSample}/image`, {
            signal: controller.signal,
        })
            .then(async (response) => {
                if (!response.ok)
                    throw new Error(
                        "This sample image is unavailable. Choose another catalogue item or upload a photo.",
                    )
                const blob = await response.blob()
                if (controller.signal.aborted) return
                const sample = new File(
                    [blob],
                    `catalogue-${initialSample}.jpg`,
                    {
                ...
                await runAnalysis(sample, controller.signal)
            })
            .catch((error) => {
    ...
    function analyse() {
        ...
        activeRequest.current = controller
        void runAnalysis(file, controller.signal)
    }
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                {result
                                    ? "The strongest prediction for each attribute"
                    ...
                    <div className="mt-16" aria-live="polite">
                        <div className="mb-6">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Top model outputs and confidence scores.
                            </p>
                        </div>
                        ...
                        <div className="mb-6 mt-16 flex items-end justify-between gap-4">
                            ...
                            <p className="text-sm text-muted-foreground">
                                {result.similar_items.length} matches
                            </p>
                        </div>

...
These copies retain Tasks 1–3 and add three CNN filter alternatives plus two alternative architectures for each MLP family. Training code stays inside each notebook; `run_all.py` only executes them sequentially in fresh kernels. Production notebooks and artifacts are not replaced.
...
The notebooks also compare shallow Dense(64) and Dense(128) alternatives against Dense(256), and deeper Dense(128, 64, 32) and Dense(256, 128) alternatives against Dense(256, 128, 64). All use the original input size and dropout 0.2. The architecture references are the existing lower-learning-rate MLPs: Adam 1e-4 for shallow and 3e-4 for deeper, up to 20 epochs with patience 4.

There are **16 candidates per target, 64 total**, but **only missing candidate files are trained**. If all earlier experiments are present, this adds only 16 MLP fits across the four targets. Every training block first checks `models-test/` and reloads existing models and histories. Existing CNNs are not retrained. Missing CNNs, including targets that never finished, still need their initial training. Mismatched or incomplete existing exports cause an error instead of silent retraining. Selection uses validation macro-F1, then accuracy, then parameter count. Only the selected candidate receives calibration and internal-test evaluation; internal-test results must not guide architecture selection. There is one seed, so small improvements are provisional.

## Outputs

- `models-test/<target>_<method>.keras`: all 64 candidates after completion, saved immediately after fitting with restored best weights. These are uncalibrated inference exports, without optimizer slots. Existing candidate files are reused.
...
The runner stops on the first error and preserves completed outputs and candidate exports. Running the same command again skips notebooks it previously completed with unchanged source and model files still present. An edited or failed notebook executes from the beginning, but its training blocks reuse existing candidate exports and train only missing files. Histories and configuration JSONs in `results-test/` are required for reuse; do not delete them. Individual unfinished fits cannot resume from optimizer checkpoints. Direct VS Code executions are not recorded as runner completions, but their candidate files are still reused.

```bash
# Run only selected tasks:
python notebooks-test/run_all.py --tasks 2 3

# Refresh evaluation and selected exports, still reusing existing candidates:

"""Build and evaluate garment-aware search artifacts directly in models by default."""
...
def rebuild(model_dir: Path, data_root: Path, output_dir: Path, sample_size: int):
    ...
    (results_dir / 'visual_search_colour_evaluation.json').write_text(json.dumps(report, indent=2) + '\n')
    pd.DataFrame([
        {'method': report['previous_method'], **report['previous']},
        {'method': 'garment_hog_colour', **report['garment_hog_colour']},
    ]).to_csv(results_dir / 'visual_search_history.csv', index=False)

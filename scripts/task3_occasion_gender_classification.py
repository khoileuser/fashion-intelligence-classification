"""Run Task 3 models or create the four-column assignment prediction CSV."""
...
from preprocessing import load_prediction_template
...
def classify(image_path: str | Path, target: str) -> dict:
    with Image.open(image_path) as image:
        result = FashionClassifier(MODEL_PATHS[target]).predict(image)
    print(result)
    return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", choices=("gender", "usage"))
    parser.add_argument("--submission", action="store_true")
    parser.add_argument("--output", default=str(ROOT / "prediction" / "styles_prediction.csv"))
    args = parser.parse_args()
    if args.submission:
        create_submission(args.output)
    elif args.image and args.target:
        classify(args.image, args.target)
    else:
        parser.error("provide --image with --target, or use --submission")
